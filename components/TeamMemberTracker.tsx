      ...
      <div className="flex-grow overflow-auto">
        {teamMembers.length > 0 ? (
          <>
            ...
            <div className="md:hidden space-y-4">
              {teamMembers.map(member => (
                <div key={member.id} className="bg-white p-4 rounded-lg shadow border border-black flex flex-col space-y-3">
                  ...
                  <button onClick={() => handleEditClick(member)} className="mt-3 w-full text-center px-4 py-2 bg-white border border-black rounded-md text-sm font-medium text-black hover:bg-gray-100">
                    Edit / Delete
                  </button>
                </div>
              ))}
            ...
            <div className="hidden md:block border border-black rounded-lg overflow-hidden">
              <table className="min-w-full divide-y divide-black">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Name / EID</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Role</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Salary</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Current Project</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Joined</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Projects Done</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {teamMembers.map((member) => (
                    <tr key={member.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{member.role}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{formatCurrency(member.salary)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{member.currentProjectFor || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{formatDate(member.joinedDate)}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-bold text-black">{member.projectsDoneThisMonth}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={() => handleEditClick(member)} className="text-black hover:text-gray-700 font-bold">Edit</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="text-center py-16 px-6 h-full flex flex-col justify-center items-center border border-dashed border-black rounded-lg">
            <UserCircleIcon className="mx-auto h-12 w-12 text-black" />
            <h3 className="mt-2 text-lg font-medium text-black">No team members yet</h3>
            <p className="mt-1 text-sm text-black">Get started by adding a new team member.</p>
          </div>
        )}
      </div>
