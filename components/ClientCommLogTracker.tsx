...
const FileLink: React.FC<{file: FileInfo}> = ({ file }) => (
    <a href={file.dataUrl} download={file.name} className="text-gray-500 hover:text-blue-600 inline-block" title={`Download ${file.name}`}>
        ...
        {logs.length > 0 ? (
          <>
            ...
            <div className="md:hidden space-y-4">
              {logs.map(log => (
                <div key={log.id} className="bg-white p-4 rounded-lg shadow-md border border-gray-200 flex flex-col space-y-3">
                    <div className="flex-grow">
                        ...
                        <p className="text-sm text-gray-600 mt-1">{log.siteLocation}</p>
                  ...
                  <button onClick={() => handleEditClick(log)} className="mt-3 w-full text-center px-4 py-2 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">
                    Edit / Delete
                  </button>
                </div>
              ))}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Floor Plan</th>
                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Quote</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Comm. Images</th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Other Docs</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{log.siteLocation}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{log.floorPlan ? <FileLink file={log.floorPlan} /> : 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{log.quote ? <FileLink file={log.quote} /> : 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{log.commImages.length} file(s)</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{log.otherDocs.length} file(s)</td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <button onClick={() => handleEditClick(log)} className="text-blue-600 hover:text-blue-800 font-semibold">Edit</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="text-center py-16 px-6 h-full flex flex-col justify-center items-center border-2 border-dashed border-gray-300 rounded-lg">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900">No communication logs yet</h3>
            <p className="mt-1 text-sm text-gray-500">Get started by adding a new log.</p>
          </div>
        )}
      </div>
